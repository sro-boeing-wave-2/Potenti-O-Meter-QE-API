...
namespace QuizServer.Service
{
    public class GraphService : IGraphService
    {
        ...
        public List<ContentRecommender> GetContentRecommendations(int userId, string domain)
        {
            ...
            using (ISession session = driver.Session())
            {
                result1 = session.Run("match (c:Concept)-[:Concept_Of]->(:Domain{name:\"" + domain + "\"}) WITH COLLECT (DISTINCT c) as ccoll  Match(co:Content)-[r] -> (cprime: Concept) where cprime in ccoll return co,cprime limit 6");
